...
def run_early_fusion_cv(
    ...
    results_dir: str = "results/hybrid/early_fusion"
) -> Dict:
    """
...
def run_late_fusion_cv(
    ...
    results_dir: str = "results/hybrid/late_fusion"
) -> Dict:
    """
    ...
    for val_fold in range(n_folds):
        ...
        y_val = fold_data[CFG.target_col].values.astype(int)
...
def run_stacking_fusion_cv(
    ...
    fold_df: pd.DataFrame,
    results_dir: str = "results/hybrid/stacking"
) -> Dict:
    """
    ...
    for val_fold in range(n_folds):
        ...
        y_meta_train = meta_train[CFG.target_col].values.astype(int)

        X_meta_val = meta_val[
            ['oof_prob_brugada_hc', 'oof_prob_brugada_cnn']
        ].values
        y_meta_val = meta_val[CFG.target_col].values.astype(int)
...
def _align_oof_predictions(
    ...
    # Merge on patient_id
    merged = oof_hc[['patient_id', CFG.target_col,
    ...
    label_mismatch = (merged[CFG.target_col] !=
                      oof_cnn.merge(merged[['patient_id']],
                      on='patient_id')[CFG.target_col].values
                      if CFG.target_col in oof_cnn.columns else False)
