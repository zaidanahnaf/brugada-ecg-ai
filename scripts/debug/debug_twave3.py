...
def simulate_t_window(median_rr_ms, fs=100):
    ...
    pre  = int(CFG.preprocessing.beat_window_pre_ms  * fs / 1000)   # 20
    post = int(CFG.preprocessing.beat_window_post_ms * fs / 1000)   # 50
    ...
    if median_rr_ms > 400:
        ...
        t_s_hr  = r_sample + int(CFG.preprocessing.t_wave_start_fraction * rr_s)
        t_e_hr  = r_sample + int(CFG.preprocessing.t_wave_end_fraction   * rr_s)
    ...
    # Fallback
    t_s = r_sample + int(CFG.preprocessing.t_wave_fallback_start_ms * fs / 1000)
    t_e = r_sample + int(CFG.preprocessing.t_wave_fallback_end_ms   * fs / 1000)
...
print(f"beat_window_pre_ms         : {CFG.preprocessing.beat_window_pre_ms}")
print(f"beat_window_post_ms        : {CFG.preprocessing.beat_window_post_ms}")
print(f"t_wave_fallback_start_ms   : {CFG.preprocessing.t_wave_fallback_start_ms}")
print(f"t_wave_fallback_end_ms     : {CFG.preprocessing.t_wave_fallback_end_ms}")

pre  = int(CFG.preprocessing.beat_window_pre_ms  * 100 / 1000)
post = int(CFG.preprocessing.beat_window_post_ms * 100 / 1000)
