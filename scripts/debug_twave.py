...
def debug_one_subject(patient_id: str):
    ...
    pre_samples = int(CFG.beat_window_pre_ms * record['fs'] / 1000)
    post_samples = int(CFG.beat_window_post_ms * record['fs'] / 1000)
    ...
    if t_end > t_start:
        ...
        t_start_expected = fid['r_sample'] + int(CFG.t_wave_start_fraction * rr_samples)
        t_end_expected   = fid['r_sample'] + int(CFG.t_wave_end_fraction * rr_samples)
        ...
        print(f"post_window (ms)  : {CFG.beat_window_post_ms}")
