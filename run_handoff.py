...
def best_model_factory():
    return LogisticRegression(
        ...
        max_iter=2000, random_state=CFG.feature.random_seed
    )
...
manifest = generate_full_manifest(
    ...
    save_path="outputs/features/feature_manifest.json"
)
...
try:
    consensus = pd.read_csv("outputs/results/interpretability/consensus_importance.csv")
    export_top_k_feature_set(
        feature_df=feature_df,
        consensus_importance=consensus,
        k_values=[10, 20, 35],
        output_dir="outputs/features/reduced"
    )
...
save_oof_predictions(
    ...
    save_path="outputs/features/oof_predictions_handcrafted.csv"
)
...
cnn_emb_path = "outputs/features/cnn_embeddings.csv"
cnn_oof_path = "outputs/features/cnn_fold_probs.csv"
